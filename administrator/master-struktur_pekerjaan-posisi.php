...
<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
            ...
            <h1 class="page-header"><i class="fa fa-bar-chart-o"></i> Struktur Pekerjaan - Posisi</h1>
    </div>
    <ol class="breadcrumb">
...
<script type="text/javascript">
    ...
    function bootBox(bootModal){
        bootModal.on('shown.bs.modal', function(event) {
            ...
            {
                $.ajax({
                    url: 'master-struktur_pekerjaan-source?tb=posisi_kerja',
                    dataType: 'json'
                })
                .done(function(data) {
                    console.log("success");
    ...
    $(document).ready(function() {
        data = $('#my-table').dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "master-struktur_pekerjaan-tbl?source=posisi_kerja",
            "bPaginate": true,
            "bSort": true,
            "bAutoWidth": false,
        ...
        $('.tambah').click(function(event) {
            bootModal = bootbox.prompt("Posisi", function(result) {                
                if (result === null) {                                             
                    ...
                    {
                        $.ajax({
                            url: 'master-struktur_pekerjaan-act?act=tambah&tb=posisi_kerja',
                            type: 'POST',
                            data: {
                                dataValue: result,
                ...
                buttons: {
                    ...
                    main: {
                        ...
                        callback: function() {
                            ...
                            {
                                $.ajax({
                                    url: 'master-struktur_pekerjaan-act?act=edit&tb=posisi_kerja',
                                    type: 'POST',
                                    data: {
                                        id: $('.old-id').val(),
        ...
        $('#my-table').on('click', '.delete', function(event) {
            ...
            bootbox.confirm("Yakin hapus data ini?", function(result) {
                ...
                {
                    $.ajax({
                        url: 'master-struktur_pekerjaan-act?act=hapus&tb=posisi_kerja',
                        type: 'POST',
                        data: {
                            id: that.data('id'),
                            auth: '<?php echo $_SESSION["loginID"] ?>'
